    ...
    public class SoapEndpointMiddleware
    {
        ...
        public async Task Invoke(HttpContext httpContext, IServiceProvider serviceProvider)
        {
            ...
            if (httpContext.Request.Path.Equals(_endpointPath, StringComparison.Ordinal))
            {
                ...
                responseMessage = Message.CreateMessage(_messageEncoder.MessageVersion, operation.ReplyAction,
                    bodyWriter);
        ...
        private object[] GetRequestArguments(Message requestMessage, OperationDescription operation)
        {
            ...
            using (var xmlReader = requestMessage.GetReaderAtBodyContents())
            {
                ...
                {
                    var parameterName = parameters[i].GetCustomAttribute<MessageParameterAttribute>()?.Name ??
                                        parameters[i].Name;
                    ...
                    {
                        var serializer = new DataContractSerializer(parameters[i].ParameterType, parameterName,
